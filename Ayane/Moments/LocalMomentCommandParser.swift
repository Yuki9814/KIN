...
enum LocalMomentCommandParser {
    ...
    private static func isExplicitPublishRequest(_ text: String) -> Bool {
        ...
        return firstMatch(
            lead + #"(?:发|发布|更新|晒|创建)(?:\s|一下|一个|一条|个|条|一篇)*朋友圈"#,
            in: text
        ) != nil
            || firstMatch(
                lead + #"朋友圈\s*(?:发|发布|更新|创建)(?:一下|一个|一条|个|条)?"#,
                in: text
            ) != nil
    }
    ...
    private static func isNegatedOrInformational(_ text: String) -> Bool {
        let negativePatterns = [
            #"(?:不要|别|不用|不必|禁止|不准).{0,8}(?:发|发布|更新|晒|创建).{0,4}朋友圈"#,
            #"(?:不想|不希望|没想过).{0,12}(?:发|发布|更新|晒|创建).{0,4}朋友圈"#,
            #"取消.{0,8}朋友圈"#,
            #"(?:如果|假如|要是|除非).{0,16}(?:发|发布|更新|晒|创建).{0,4}朋友圈"#,
            #"(?:关于|讨论|聊聊|解释).{0,10}(?:发|发布|更新|晒|创建).{0,4}朋友圈"#,
            #"(?:怎么|如何).{0,8}(?:发|发布|更新|创建).{0,4}朋友圈"#,
            #"为什么.{0,12}(?:发|发布|更新|创建).{0,4}朋友圈"#,
            #"(?:会不会|会).{0,6}(?:发|发布|创建).{0,4}朋友圈[吗么嘛？?]"#,
            #"(?:我|他|她).{0,4}(?:刚刚?|已经|昨天|前天|上次).{0,5}(?:发|发布|更新|晒|创建).{0,4}朋友圈"#,
            #"(?:我|他|她).{0,3}(?:发|发布|更新|晒|创建)(?:了|过).{0,4}朋友圈"#,
            #"(?:我|他|她).{0,6}(?:发|发布|更新|晒|创建).{0,4}朋友圈(?:了|过)?[吗么嘛？?]"#,
            #"朋友圈.{0,4}(?:发|发布|更新|创建)(?:过|完|好)?了"#,
            #"朋友圈.{0,4}(?:发|发布|更新|创建).{0,3}(?:了吗|过吗|没有|没)[？?]?"#,
            #"(?:发|发布|更新|晒|创建).{0,4}朋友圈.{0,8}(?:了吗|过吗|没有|吗|么|嘛)[？?]?$"#
        ]
