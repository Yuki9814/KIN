...
兼容边界是显式单向的：ArchivePayloadCodec.decodePortableOrApple 可读取 Apple AyaneDataExport schema v4–v16 的脱敏 JSON，并把 profiles、relationships、events、memories、tombstones 与事件内嵌附件转换为 KMP core；Apple 专属 evidence、summaries、Moments、群聊、主动任务、OAuth、用户资料和 world profile 会被忽略。KMP 导出的 canonical KINPortableArchivePayloadV1 不是 Apple AyaneDataExport JSON，KMP→Swift 的同构转换需由 Swift 侧按本节 schema/fixture 实现，当前不宣称双向无损互导。
